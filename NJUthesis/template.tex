...
\item 冷启动问题。该方法顺利工作的前提是用户已经产生了一些行为，即用户已经在当前会话期内收听了一定数量的歌曲。也就是说，朴素的基于多维时间序列分析的音乐推荐方法适合在用户
状态稳定时为用户做出合理的推荐。但是，对于刚进入给定平台的用户，朴素方法的推荐效果就要大打折扣，这是因为用户刚进入给定平台时所产生的行为很少，而对应的听歌序列还
不足以被朴素的多维时序方法进行分析和预测。
\end{enumerate}
...
可以概括为三种基本设计思想：整体式综合设计、并行式综合设计和流水线式综合设计，本文下面将对这三种设计思想进行简单介绍并给出本文所提基于用户三期行为的综合音乐推荐
方法所采用的设计思想。

\subsection{整体式综合}

整体式设计就是将几种不同推荐策略整合到一个算法中实现的综合设计，实际上确实所有推荐策略都在发挥作用，如图4.2所示。举例来说，可以通过基于用户即时行为的推荐
得到用户可能收听的下一首歌曲在K个隐含主题上的概率分布，而同样也可以通过基于用户长期行为推荐和基于用户中期行为推荐得到用户可能收听的下一首歌曲在K各隐含主题上的
概率分布。整体式推荐的一个简单实现就是在得到三个概率分布的基础上，综合得到最终的概率分布并以此来产生推荐列表。
...
并行式设计就是不同的推荐算法根据输入并行运行分别产生推荐列表，最后将这些推荐列表混合产生最终的推荐列表推荐给用户的设计思路，如图4.3所示。举例来说，并行式设计需要
分别利用用户的长期行为、中期行为和即时行为分别为用户生成一个推荐列表，然后将这三个列表进行混合以生成最终的推荐结果。至于综合的策略，可以采用加权、交叉以及切换等
进行混合。交叉式混合推荐在在用户交互界面层面上将不同的推荐结果组合到一起，各种方法所得到的结果一同被呈现。加权式混合推荐关键字是加权，通过计算多个推荐结果分数的
...
如图4.4所示，流水线式设计将多个推荐策略按照流水线架构连接起来，前一个推荐系统的输出变成后一个推荐系统的输入部分。当然，后面的推荐单元也可以选择使用部分原始
输入数据。
...
不同的推荐策略进行推荐并最终得到推荐结果。显然，本文所述的音乐推荐不具有明确的阶段划分，因此使用流水线式的混合设计不太合适，而整体式混合设计和并行式混合设计均
可应用，只是二者混合的阶段不一致。由于整体式的混合设计思路是从内部对不同推荐策略的结果进行混合，我们认为这样的早期混合能够得到比较符合用户偏好的特征，因此我们
主要考察这种混合设计方法，如图4.5所示。
...
这里，我们分别给出三种不同的混合方法，分别是：基于用户行为局部性特征的推荐与基于用户行为时序性特征的推荐相混合的混合推荐方法“seq\_local”，如公式4.3所示；基于用户
行为全局性特征的推荐与基于用户行为时序性特征的推荐相混合的混合推荐方法“seq\_global”，如公式4.4所示；基于用户行为三元特征的混合推荐方法“all”，如公式4.5所示。

\begin{equation}
{\hat{\omega}}_{i}(\pi+n+1)=\alpha{\hat{\omega}}_{i\_mtsa}(\pi+n+1)+(1-\alpha)(\beta{\hat{\omega}}_{i\_local}(\pi+n+1)+(1-\beta){\hat{\omega}}_{i\_global}(\pi+n+1))
\end{equation}

其中,表示基于用户时序性特征的推荐方法的权重，表示在完全混合推荐方法中基于用户行为局部性特征的推荐方法的权重。对于公式4.3和4.4，我们按照公式4.6定义。

\begin{equation}
\alpha=\frac{f(n-b)}{n+a}
\end{equation}

其中，n表示待考察歌曲序列的长度，即用户在当前会话期内所喜好的歌曲数目；参数b用以表征时序性特征其作用的起始长度；参数a用以调整时序性特征作用的变化速率。算子f的
定义如公式4.7所示：
...
图4.6给出了当a=10且b=5时，的取值变化情况，其中横坐标表示序列长度，纵坐标表示的取值。由图可以看出，当序列长度小于5时=0说明序列长度过短时不考虑时序特征；
而随着序列长度的增加，的取值不断增加，即时序性特征的作用越来越大。
...
为了评估本节所给出的混合推荐方法的效果，我们将之与第三章中给出的单纯的基于时间序列分析的音乐推荐方法进行比较，实验所采用的数据集和实验设置和3.6.3节中所述一致，
而评测指标则和3.6.2中所述一致即综合采用recall、precision、RMSE和MAE进行评估。

图4.6给出了几种不同的推荐算法随着推荐列表长度的增加命中率的变化情况。其中，MTSA表示单纯的基于时间序列分析的音乐推荐算法，MTSA\_Local表示基于用户行为时序性特征
和局部性特征的混合推荐方法，MTSA\_Global表示基于用户行为时序性特征和全局性特征的混合推荐方法，MTSA\_ALL表示基于用户行为三元特征的混合推荐方法。由图可以看出，
经过混合的推荐能够取得比单一的推荐更好地命中率。由于通过各个推荐算法的准确度的变化情况也可以得到类似的结论，这里就不再赘述。
...
图4.7给出了几种不同的推荐算法随着推荐列表长度的增加均方根误差的变化情况。由图可以看出，几种混合的推荐方法的均方根误差都在0.5左右，而单一的MTSA均方根误差
在3.25左右，这说明经过混合的推荐能够取得比单一的推荐更低的推荐误差且降低的幅度比较大。图4.8更加清晰地展示了几种混合推荐方法之间均方根误差的变化情况。
由于通过各个推荐算法的绝对误差的变化情况也可以得到类似的结论，这里就不再赘述。

\section{分析讨论}

本文上面分别介绍了本文所提的一种基于时间序列分析的个性化音乐推荐方法以及以此为基础构建的混合推荐框架，本节将其中一些问题进行分析和讨论。

\subsection{最长可分析序列}

本文所述的基于时间序列分析的个性化音乐推荐方法及框架主要是用户在当前会话期内所收听(喜欢)的歌曲构成的序列进行分析和预测，那么如果用户在当前会话期内收听了很多首歌
曲(假如是1000首)，我们是不是要对所有这些歌曲构成的序列进行分析进行分析呢？显然，这么做既不划算也不合理，因为用户所处的上下文环境是时刻改变的，很久之前的行为对当
前环境状态的表征作用就很有限甚至会形成干扰。为此，我们设置一个“最大序列长度”，这样我们只需要对一定范围内的序列进行分析即可，既减轻了计算的负担同时也排除了老旧数
据的干扰。顾名思义，“最大序列长度”就是算法所能够考察的用户行为序列的最大长度。假如设置最大序列长度为4且用户依次收听了歌曲s1、s2、s3、s4、s5、s6、s7,那么算法只分
析由s4、s5、s6、s7构成的序列而忽略s1、s2、s3。如果调整最大序列长度为3，那么歌曲s4也将被忽略，算法只考察由s5、s6、s7构成的序列。图4.9展示了随着最大序列长度的增加，
基于用户行为局部特征的音乐推荐算法的命中率变化情况(推荐列表长度设为100)，图中横坐标表示最大序列长度，纵坐标表示推荐算法的命中率。由图可以看出，对于当前数据集来说，
当最大序列长度为15时，算法的命中率基本就保持不变了，这说明更早之前的数据对提升命中率意义不大。因此，可以取最大序列长度为15。当然，不同的数据集最大序列长度取值不
一样，但差别不大，一般取15~30为宜。

\begin{figure}[!h]
\begin{center}
\includegraphics [width=0.75\textwidth]{figures/windowsize.eps}
\caption{不同最长可分析序列对应的命中率}
...
第二章中介绍了向量空间模型、词频-逆文本频率模型、隐含狄利克雷分配等几种不同的文本建模方法，同时提到LDA模型与VSM模型和TF-IDF模型相比，一方面能够更好地描述文档的
文本特征，另一方面可以解决一词多义和一义多词的问题，因此本文所述的基于多维时间序列分析的音乐推荐方法和基于用户三期行为的音乐推荐方法在对歌曲集对应的文档集进行建
模时都采用了LDA模型。为了进一步地展现LDA模型的好处，本节通过实验来验证使用不同的文本建模方法来执行基于用户即时行为推荐所获得的命中率，如下图所示。其中，横坐标表示
推荐列表的长度，纵坐标表示算法获得的命中率。纵坐标取值越大，算法获得命中率越高，算法推荐效果越好。由图可以非常直观地看出，使用隐含狄利克雷分配对文档集进行建模确实
能够得到比传统的向量空间模型和词频-逆文本频率更好的效果。
