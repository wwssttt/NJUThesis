...
\begin{englishabstract}
    ...
    douban.fm. However, the recommender algorithms behind most of these applications do not take the relationship between users' listening behaviors
    and the contexts into consideration leading to unsatisfactory recommendations and a few work that take the relationship into consideration do not
    get good recommendations as well because of not handling the time-related features of users' behaviors in a reasonable way. We think users' future behaviors
    are affected by long-term behaviors、mid-term behaviors and immediate behaviors at the same time, but there is few work handling the time-related features
    of users' behaviors comprehensively. As a result, we propose a music recommender method based on multidimensional time series analysis to enhance recommender
    effects by handling users' mid-term behaviors in a reasonable way and then we propose a comprehensive music recommender method that takes users' long-term
    behaviors、mid-term behaviors and immediate behaviors into consideration. What's more, we implement a system prototype to preliminarily verify the feasibility
    of our methods. Our main contribution and work are as follows:

    1. A music recommender method based on multidimensional time series analysis was proposed. The method firstly uses topic models to model a song as
    a probability distribution of some latent topics and then models a user's behaviors as a multidimensional time series. By analyzing the time series,
    the method can forecast the user's future behaviors and get reasonable recommendations.

    2. A music recommender method based on users' long-term behaviors、mid-term behaviors and immediate behaviors was proposed. The method can adjust the weights
    of these behaviors dynamically and get better recommendations than the pure music method based on multidimensional time series analysis.

    3. A music recommender system prototype based on above methods was implemented. In order to improve the efficiency, we use Storm, a distributed realtime
...
一种基于用户长期、中期和即时行为的综合音乐推荐方法，全面地考察用户长期行为、即时行为和中期行为的贡献和作用。在给出基于多维时间序列分析的音乐推荐方法和基于用户长期、中期和即时行为的综合
...
方法的基础上进一步给出的一种基于用户长期、中期和即时行为的综合推荐方法；第5章给出了本文所实现的原型系统及相关实现技术；第6章对全文进行总结并给出对未来工作的展望。
...
序列分析的音乐推荐方法的基础上，进一步给出一种基于用户长期、中期和即时行为的综合推荐方法，综合考虑用户三期行为的贡献和作用。
...
本文所提出的基于多维时间序列分析的音乐推荐方法及在此基础上给出的基于用户长期、中期和即时行为的综合推荐方法将使用主题模型建模的方法将每一首歌曲表征为由若干隐含主题构成的一个概率
...
为了解决上述两个问题，我们将在本章中给出一种基于用户长期、中期和即时行为的综合音乐推荐方法，用户的三期行为即用户的长期行为、中期行为和即时行为。在介绍了这种基于用户三期行为
...
可以概括为三种基本设计思想：整体式综合设计、并行式综合设计和流水线式综合设计，本节下面将对这三种设计思想进行简单介绍并给出本文所提出的基于用户长期、中期和即时行为的综合音乐
推荐方法所采用的设计思想。其中，整体式混合设计实际上是一种推荐算法，其在推荐内部对不同推荐策略获取的特征进行混合，然后基于此做出最终推荐列表；并行式混合设计和流水线式
...
图4.6展示了基于用户长期、中期和即时行为的综合音乐推荐和朴素的基于多维时间序列分析的音乐推荐方法在命中率指标上的表现。其中，横坐标表示推荐列表的长度，纵坐标表示推荐命中率，MTSA
表示朴素的时序推荐方法，MTSA\_ALL表示综合推荐方法。由图可以非常直观地看出，基于用户长期、中期和即时行为的综合音乐推荐方法能够取得比朴素的时序分析方法更高的命中率。图4.7展示了这
...
进一步地，本文认为用户的未来行为除了受到用户中期行为的影响外还受到用户长期行为和即时行为的共同作用。在此直觉下，本文给出了一种基于用户长期、中期和即时行为的综合音乐推荐方法，综合考虑了用户长期行为、中期行为以及即时行为对用户未来行为的作用和贡献。

在给出了朴素的基于多维时序分析的音乐推荐方法和基于用户长期、中期和即时行为的综合音乐推荐方法之后，本文设计了实验并从不同的角度验证了所述方法的有效性，这主要体现在本文所给方法能够取得比参考方法更高的推荐命中率以及更低的预测误差。此外，本文实现了一个原型系统以验证所述方法的可行性。在原型系统中，我们实现了所述音乐推荐方法的并行化，提升了系统的执行效率。
